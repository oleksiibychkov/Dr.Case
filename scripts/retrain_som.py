"""
Dr.Case — Перенавчання SOM на об'єднаних даних (863 хвороби)
...
def main():
    ...
    if not database_path.exists():
        ...
        print("   Покладіть unified_disease_symptom_merged.json в папку data/")
        return
    
    print(f"\n📁 База даних: {database_path}")
    print(f"📁 Вихідна модель: {output_model_path}")
    
    # Конфігурація SOM (оптимізована для 863 хвороб)
    # Формула Vesanto: grid_size ≈ 5 * sqrt(N) = 5 * sqrt(863) ≈ 147 → 12x12 або 15x15
    config = SOMConfig(
        grid_height=30,          # Збільшуємо для кращого розподілу
        grid_width=30,
        input_dim=466,           # Кількість симптомів
        epochs=500,
