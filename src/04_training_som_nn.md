...
3. Для кожної епохи і для кожного \\(x_d\\):
   - знайти BMU: \\(u^{\ast} = \arg\min_u \|x_d - w_u\|\\)
   - оновити BMU і сусідів:
     \\[w_u \leftarrow w_u + \eta(t) \cdot h(u, u^{\ast}, \sigma(t)) \cdot (x_d - w_u)\\]
...
### 34.4. Функція сусідства \\(h(u, u^{\ast}, \sigma)\\)

**Гаусова функція (стандарт):**
\\[h(u, u^{\ast}, \sigma) = \exp\left(-\frac{\|r_u - r_{u^{\ast}}\|^2}{2\sigma^2}\right)\\]

де \\(r_u\\) — координати юніта \\(u\\) на карті.

**Bubble-функція (спрощена):**
\\[h(u, u^{\ast}, \sigma) = \begin{cases} 1 & \text{якщо } \|r_u - r_{u^{\ast}}\| \leq \sigma \\ 0 & \text{інакше} \end{cases}\\]
