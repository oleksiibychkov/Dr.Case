...
Нехай істинний діагноз пацієнта — \\(d^{\ast}\\). "Не втрачено" означає:
\\[d^{\ast} \in \mathcal{D}_{cand}(x_p)\\]

---

## 16. Ключова лема (умова достатності)

**Лема.** Якщо існує юніт \\(u^{\ast} \in S(x_p)\\) такий, що \\(d^{\ast} \in I(u^{\ast})\\), то \\(d^{\ast} \in \mathcal{D}_{cand}(x_p)\\).

**Доведення.**

За означенням \\(\mathcal{D}_{cand}(x_p) = \bigcup_{u \in S(x_p)} I(u)\\).

Якщо \\(u^{\ast} \in S(x_p)\\), то \\(I(u^{\ast})\\) входить у це об'єднання.

Якщо \\(d^{\ast} \in I(u^{\ast})\\), то \\(d^{\ast}\\) належить об'єднанню. ∎

Тобто вся задача зводиться до: **гарантувати, що Candidate Selector включив хоча б той юніт, до якого прив'язаний \\(d^{\ast}\\)**.
...
1. Пацієнтський вектор близький до вектора істинного діагнозу:
   \\[\|x_p - x_{d^{\ast}}\| \leq \varepsilon_{\text{case}}\\]
   (це формалізує "пацієнт описав симптоми без великої помилки".)

2. Для діагнозу \\(d^{\ast}\\) SOM має обмежену квантизаційну помилку:
   \\[\|x_{d^{\ast}} - w_{u^{\ast}}\| \leq \varepsilon_{\text{som}}\\]
   де \\(u^{\ast} = \mathrm{BMU}(x_{d^{\ast}})\\).
...
Нехай \\(u^{\ast} = \mathrm{BMU}(x_{d^{\ast}})\\). За побудовою мапи на кроці 5:
\\[d^{\ast} \in I(u^{\ast}).\\]

Оцінимо відстань від пацієнта до прототипу цього юніта:
\\[\|x_p - w_{u^{\ast}}\| \leq \|x_p - x_{d^{\ast}}\| + \|x_{d^{\ast}} - w_{u^{\ast}}\| \leq \varepsilon_{\text{case}} + \varepsilon_{\text{som}} \leq r\\]
(трикутна нерівність).

Отже \\(u^{\ast}\\) належить множині юнітів в радіусі \\(r\\), а значить \\(u^{\ast} \in S(x_p)\\).
Тоді за Лемою \\(d^{\ast} \in \mathcal{D}_{cand}(x_p)\\). ∎
...
Нехай \\(u^{\ast} = \mathrm{BMU}(x_{d^{\ast}})\\). Якщо \\(u^{\ast}\\) входить у top-k юнітів для \\(x_p\\), тоді істинний діагноз не втрачається:
\\[u^{\ast} \in \mathrm{TopK}(x_p) \Rightarrow d^{\ast} \in \mathcal{D}_{cand}(x_p)\\]

бо \\(d^{\ast} \in I(u^{\ast})\\) (крок 5) і \\(u^{\ast} \in S(x_p)\\) (вибір).

### 18.2. Практично-формальна умова "розумного k"

Якщо існує розрив (separation) між відстанню до \\(u^{\ast}\\) та відстанями до більшості інших юнітів, тобто кількість юнітів ближчих за \\(u^{\ast}\\) не перевищує \\(k-1\\), тоді \\(u^{\ast}\\) буде в top-k. Формально:
\\[\#\\{u \in U:\ \|x_p - w_u\| < \|x_p - w_{u^{\ast}}\|\\} \leq k - 1.\\]
...
**Що таке "істинний юніт" \\(u^{\ast}\\)**

Нехай істинний діагноз пацієнта — \\(d^{\ast}\\). На офлайн-кроці 5 ви прив'язали кожен діагноз з бази до одного юніта. Отже існує юніт \\(u^{\ast}\\), такий що:
\\[d^{\ast} \in I(u^{\ast})\\]

Саме \\(u^{\ast}\\) ми називаємо **істинним юнітом** для цього діагнозу.
...
Нехай \\(u^{\ast}\\) — деякий юніт з цього списку, і його membership позначимо \\(m(u^{\ast})\\).
...
Це твердження **достатнє**: якщо умова виконується, включення гарантоване. Якщо умова не виконується, \\(u^{\ast}\\) може все одно входити в \\(S_\alpha\\), але теорема цього не гарантує.

### 21.3. Повне доведення (крок за кроком)

Почнемо з того, що ми хочемо довести включення \\(u^{\ast}\\) у множину \\(S_\alpha\\). Нагадаю:
\\[S_\alpha = \\{u_1, u_2, \ldots, u_{K_\alpha}\\}\\]
де \\(K_\alpha\\) — найменше число, для якого сума перших \\(K_\alpha\\) membership не менша за \\(\alpha\\).

Отже, твердження \\(u^{\ast} \in S_\alpha\\) еквівалентне твердженню:

> **юніт \\(u^{\ast}\\) знаходиться серед перших \\(K_\alpha\\) елементів відсортованого списку.**

Позначимо індекс юніта \\(u^{\ast}\\) у впорядкованому списку як \\(t\\). Тобто:
\\[u^{\ast} = u_t, \quad m(u^{\ast}) = m_t\\]
...
Оскільки \\(\sum_{i=1}^{n} m_i = 1\\), то сума membership усіх юнітів, крім \\(u^{\ast}\\), дорівнює:
\\[\sum_{i \neq t} m_i = 1 - m_t\\]
...
> **Сумарна membership усіх юнітів, крім \\(u^{\ast}\\), строго менша за \\(\alpha\\).**
...
Це означає, що навіть після того, як ми взяли перші \\(K_\alpha\\) юнітів, \\(u^{\ast}\\) серед них немає. Тобто індекс \\(t\\) істинного юніта більший за \\(K_\alpha\\):
\\[t > K_\alpha\\]

Інакше кажучи, ми набрали потрібну масу \\(\alpha\\), використовуючи лише юніти з множини:
\\[\\{u_1, \ldots, u_{K_\alpha}\\} \subseteq \\{u_i : i \neq t\\}\\]

Тобто ми набрали масу \\(\alpha\\) **не використовуючи \\(u^{\ast}\\)**.

**Крок 4. Порівняємо, яку масу можна набрати без \\(u^{\ast}\\)**

Якщо ми взагалі не використовуємо \\(u^{\ast}\\), то максимально можлива сумарна membership, яку ми можемо набрати, — це:
\\[\sum_{i \neq t} m_i = 1 - m_t\\]

Ми вже показали у Кроці 2, що:
\\[1 - m_t < \alpha\\]

Отже, **без \\(u^{\ast}\\) ми можемо набрати строго менше, ніж \\(\alpha\\)**.

**Крок 5. Оцінимо суму перших \\(K_\alpha\\) юнітів**

Тепер згадаємо наше припущення з Кроку 3: \\(u^{\ast} \notin S_\alpha\\), тобто \\(t > K_\alpha\\).

Це означає, що серед перших \\(K_\alpha\\) юнітів немає \\(u^{\ast}\\):
\\[\\{u_1, \ldots, u_{K_\alpha}\\} \subseteq \\{u_i : i \neq t\\}\\]

Тому сума membership перших \\(K_\alpha\\) юнітів є частиною суми всіх юнітів без \\(u^{\ast}\\):
...
Умова теореми **не виконується**, тому теорема не гарантує включення \\(u^{\ast}\\).

Справді, \\(S_{0.9}\\):
- \\(u_1\\): сума \\(= 0.85 < 0.9\\)
- \\(u_1 + u_2\\): сума \\(= 0.93 \geq 0.9\\) ✓

Отже, \\(S_{0.9} = \\{u_1, u_2\\}\\), і \\(u^{\ast} = u_3\\) **не включений**.
...
Умова теореми **виконується**, тому \\(u^{\ast} \in S_{0.9}\\) гарантовано.
...
Отже, \\(S_{0.9} = \\{u_1, u_2, u_3\\}\\), і \\(u^{\ast} = u_3\\) **включений** ✓
...
Умова виконується, тому \\(u^{\ast}\\) гарантовано в \\(S_{0.9}\\).

Перевіримо:
- \\(u_1 + u_2 + u_3 = 0.76 < 0.9\\)
- \\(u_1 + u_2 + u_3 + u_4 = 1.0 \geq 0.9\\) ✓

\\(S_{0.9} = \\{u_1, u_2, u_3, u_4\\}\\), і \\(u^{\ast}\\) **включений** ✓
...
Нехай \\(m^{\ast} = m(u^{\ast}) = 1 - \alpha\\) (граничне значення).

Визначимо класи еквівалентності:
- \\(T_{>} := \\{u \in U : m(u) > m^{\ast}\\}\\) — юніти з більшою membership
- \\(T_{=} := \\{u \in U : m(u) = m^{\ast}\\}\\) — юніти з такою самою membership
- \\(T_{<} := \\{u \in U : m(u) < m^{\ast}\\}\\) — юніти з меншою membership

### 23.2. Теорема (три випадки)

Нехай \\(m(u^{\ast}) = 1 - \alpha\\) і \\(M_{>} := \sum_{u \in T_{>}} m(u)\\).

**Випадок 1:** Якщо \\(M_{>} \geq \alpha\\), то \\(u^{\ast} \notin S_\alpha\\).

**Випадок 2:** Якщо \\(M_{>} < \alpha\\) і \\(N_{req} \geq |T_{=}|\\), де \\(N_{req} = \lceil(\alpha - M_{>})/m^{\ast}\rceil\\), то \\(u^{\ast} \in S_\alpha\\).
...
Оскільки \\(u^{\ast} \in T_{=}\\) і \\(T_{=} \cap T_{>} = \emptyset\\), маємо \\(u^{\ast} \notin S_\alpha\\). ∎

### 23.4. Доведення Випадку 2

Оскільки \\(M_{>} < \alpha\\), юнітів з \\(T_{>}\\) недостатньо для покриття маси \\(\alpha\\). Потрібно додати юніти з \\(T_{=}\\).

За умовою \\(N_{req} \geq |T_{=}|\\). Це означає, що для покриття маси \\(\alpha\\) потрібно взяти **всі** юніти з \\(T_{=}\\), включаючи \\(u^{\ast}\\). ∎
...
Якщо \\(m(u^{\ast}) \geq 1 - \alpha\\), то \\(u^{\ast} \in S_\alpha^{+}\\) **при будь-якому** початковому впорядкуванні.

### 24.3. Доведення

**Крок 1.** За означенням \\(S_\alpha^{+}\\) включає всі юніти з membership \\(\geq m_{K_\alpha}\\).

**Крок 2.** Нехай \\(m^{\ast} = m(u^{\ast})\\). Розглянемо два випадки:

**Випадок A:** \\(m^{\ast} > m_{K_\alpha}\\)

Тоді \\(u^{\ast}\\) знаходиться серед перших \\(K_\alpha\\) юнітів за означенням сортування, отже \\(u^{\ast} \in S_\alpha \subseteq S_\alpha^{+}\\).

**Випадок B:** \\(m^{\ast} \leq m_{K_\alpha}\\)

Покажемо, що \\(m^{\ast} = m_{K_\alpha}\\).

Від супротивного: нехай \\(m^{\ast} < m_{K_\alpha}\\).

Обчислимо суму membership юнітів з більшим membership ніж \\(u^{\ast}\\):
\\[\sum_{u : m(u) > m^{\ast}} m(u) \geq \sum_{i=1}^{K_\alpha} m_i \geq \alpha\\]

Але за умовою теореми \\(m^{\ast} \geq 1 - \alpha\\), отже:
\\[\sum_{u \neq u^{\ast}} m(u) = 1 - m^{\ast} \leq \alpha\\]

Суперечність.

**Крок 3.** Якщо \\(m^{\ast} \geq m_{K_\alpha}\\), то або \\(u^{\ast} \in \\{u_1, \ldots, u_{K_\alpha}\\}\\), або \\(m^{\ast} = m_{K_\alpha}\\).

У другому випадку за означенням \\(S_\alpha^{+}\\) включає всі юніти з \\(m(u) = m_{K_\alpha}\\), отже \\(u^{\ast} \in S_\alpha^{+}\\). ∎
...
\\[\mathcal{T} = \\{(c_j, d_j^{\ast})\\}_{j=1}^{N}\\]

де:
* \\(c_j\\) — клінічний випадок (симптоми),
* \\(d_j^{\ast} \in \mathcal{D}\\) — істинний діагноз.

**Candidate recall** визначаємо як:
\\[\mathrm{Rec}_{CS} = \frac{1}{N}\sum_{j=1}^{N}\mathbf{1}\\{d_j^{\ast} \in \mathcal{D}_{cand}(c_j)\\}\\]

### 25.2. Означення "поганого" кейса

Кейс \\(j\\) назвемо **поганим** (для даного \\(\alpha\\)), якщо:
\\[\max_{u \in A(d_j^{\ast})} m(u \mid x_j) < 1 - \alpha\\]
...
Для кожного "непоганого" кейса \\(j \notin \mathcal{B}_\alpha\\) існує \\(u^{\ast} \in A(d_j^{\ast})\\) такий, що \\(m(u^{\ast} \mid x_j) \geq 1 - \alpha\\).

За Теоремою 21 тоді \\(d_j^{\ast} \in \mathcal{D}_{cand}(x_j)\\), тобто індикатор \\(\mathbf{1}\\{\cdot\\} = 1\\).
...
Нехай для кейса з істинним діагнозом \\(d^{\ast}\\) існує \\(u^{\ast} \in A(d^{\ast})\\) таке, що:

1. \\(m(u^{\ast} \mid x) \geq 1 - \alpha\\) (як раніше),
2. \\(m(u^{\ast} \mid x) \geq \tau\\),
3. \\(\mathrm{rank}(u^{\ast} \mid x) \leq k\\).

Тоді \\(d^{\ast} \in \mathcal{D}_{cand}(x)\\).

### 26.2. Доведення

(1) гарантує, що \\(u^{\ast} \in S_\alpha(x)\\) (теорема 21).

(3) гарантує, що обрізання \\(\min(K_\alpha, k)\\) не викине \\(u^{\ast}\\).

(2) гарантує, що фільтр \\(m \geq \tau\\) не викине \\(u^{\ast}\\).

Отже \\(u^{\ast} \in S(x; k, \tau, \alpha)\\).

Далі як у теоремі 16: \\(d^{\ast} \in I(u^{\ast}) \subseteq \mathcal{D}_{cand}(x)\\). ∎
...
Нехай клінічні випадки \\((c, d^{\ast})\\) є випадковими величинами з деякого розподілу \\(P\\).

Визначимо подію "істина не втрачена":
\\[Z = \mathbf{1}\\{d^{\ast} \in \mathcal{D}_{cand}(x(c))\\}\\]
...
Ранг істинного юніта \\(R(x) := \mathrm{rank}(u^{\ast}(x))\\) визначає, чи буде істина в top-\\(k\\):
...
\\[s^{\ast} = \arg\max_{s \in Q} \text{IG}(s)\\]
